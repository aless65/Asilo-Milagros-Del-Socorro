...
import { ServiceService } from 'src/app/apps/habitaciones/Service/service.service';


@Component({
    selector: 'app-habiataciones-list',
  ...
  ngOnInit(): void {
    this.pageTitle = [{ label: 'Inicio', path: '/' }, { label: 'Habitacion', path: '/', active: true }];
    this._fetchData();
 ...
 */
  ...
  habitacionNameFormatter(habitacion: Habitacion): any {
    return this.sanitizer.bypassSecurityTrustHtml(
      ...
      <a href="javascript:void(0);" class="customer text-body fw-semibold" id="${habitacion.habi_Numero}">${habitacion.cate_Nombre}</a>
      `
    );
  }
  ...
  habitacionActionFormatter(habitacion: Habitacion): any {
    return this.sanitizer.bypassSecurityTrustHtml(
      ` <a href="javascript:void(0);" class="edit action-icon" id="${habitacion.habi_Id}"> <i class="mdi mdi-square-edit-outline" ></i></a>
        <a href="javascript:void(0);" class="delete action-icon" id="${habitacion.habi_Id}"> <i class="mdi mdi-delete"></i></a>`
    );
  }
...
matches(row: Habitacion, term: string) {
  return (row.habi_Numero?.toString().includes(term) ||
