  ...
  Editar() {
    ...
    this.service.getEmpleadoId(id)
      .subscribe((data: any) => {
        ...
        const fecha = data.data.empe_Nacimiento;
        const fechaObjeto = new Date(fecha);
        const fechaFormateada = fechaObjeto.toISOString().split('T')[0];
  
        console.log(fechaFormateada);
  
        this.empleado.empe_Nacimiento = fechaFormateada; // Asignar la fecha formateada al campo empe_Nacimiento
      });
  }
  

  Guardar(){
    this.service.editarEmpleado(this.empleado)
    .subscribe((data: any) => {
