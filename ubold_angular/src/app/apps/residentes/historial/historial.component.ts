...
export class HistorialComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.activatedRoute.paramMap.subscribe(params => {
            ...
            if (state) {
                this.expediente = state.expedienteData.data;
                this.historialExpediente = state.historialData.data;
    ...
    getDataUrl = (url: string): Promise<string> => {
        return new Promise((resolve, reject) => {
            ...
            img.onload = () => {
                ...
                const dataURL = canvas.toDataURL('image/jpeg'); // Adjust the format if necessary
                resolve(dataURL);
            };
            img.onerror = reject;
            img.src = url;
