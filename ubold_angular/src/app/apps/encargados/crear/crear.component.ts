...
export class CrearComponent implements OnInit {
  ...
  constructor (
    ...
    /*this.service2.getResidentes().subscribe((response: any) => {
      ...
      response.data.forEach((item: any) => {
        const resiDNI: string = item.resi_Identidad;
        const resiId: string = item.resi_Id;
        const resiNombre: string = item.nombreCompleto;
    
        if (!depaLabels.includes(resiDNI)) {
          depaLabels.push(resiDNI);
          options[resiDNI] = [];
        }
    
        options[resiDNI].push({
          value: resiId,
          label: resiNombre
        });
      });
    
      this.residente = depaLabels.map((depaNombre: string) => ({
        label: depaNombre,
        options: options[depaNombre]
      }));
    });*/
  ...
  validarYGuardar() {
    ...
    this.service.createEncargados(this.encargado).subscribe((data: any) => {
  ...
  Guardar(){
    this.service.createEncargados(this.encargado)
    .subscribe((data: any) => {
      console.log("GUARDAAA");
     /* this.router.navigate([this.returnUrl]);*/
      console.log(this.encargado );
